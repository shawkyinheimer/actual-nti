...
def build_rfm(sales: pd.DataFrame, cancels: pd.DataFrame,
    ...
    print(f"RFM customers: {len(rfm):,} | dropped returns-only (pre-window buyers): "
          ...
          f"{int(rfm['HadFullReturn'].sum())}  <- handled by Yeo-Johnson, would be NaN under log1p")
    return rfm
...
def make_transformer(method: str = "yeo-johnson"):
    ...
    if method == "log1p":
        return Pipeline([
            ("log", FunctionTransformer(lambda X: np.log1p(np.clip(X, 0, None)))),
...
# ---------------------------------------------------------------------
# 4. CLUSTER PROFILING — inverse-transform centroids back to £ / days
# ---------------------------------------------------------------------
def profile_clusters(rfm: pd.DataFrame, labels: np.ndarray,
    ...
    prof = rfm.groupby(labels)[FEATURES].mean().round(1)
    prof["customers"] = np.bincount(labels)
    prof["rev_share_%"] = (100 * rfm.groupby(labels)["Monetary"].sum()
                           / rfm["Monetary"].sum()).round(1)
    return prof.sort_values("Monetary", ascending=False)


if __name__ == "__main__":                       # smoke test
    from retail_preprocessing import run_pipeline   # ← was: from preprocessing import run_pipeline
